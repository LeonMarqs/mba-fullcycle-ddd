...
export class EventService {
  ...
  async create(input: CreateEventDTO) {
    const partner = this.partnerRepo.findById(input.partner_id);

    if (!partner) {
      throw new Error('Partner not found');
    }

    const event = (await partner).initEvent(input);
  ...
  async updateSection(input: UpdateSectionDTO) {
    const event = await this.eventRepo.findById(input.event_id);
